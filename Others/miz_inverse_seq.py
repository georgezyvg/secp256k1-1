...
def hunt(start, stop, add, mag, cores='all'):
    ...
    try:
        ...
        for r in range(cores):
            p = Process(target=main, args=(counter, start, stop, add, mag))
            workers.append(p)
            p.start()
...
def main(counter, start, stop, add, mag):
    count=0
    iteration = 0
...
if __name__ == '__main__':
    ...
    print("==========================================================")
    print('Total Bitcoin Addresses Loaded and Checking : ',str (line_count))
    hunt(start, stop, add, mag, cores = 1)